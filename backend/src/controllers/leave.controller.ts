    ...
    if (leave.employee.managerId) {
      ...
      if (manager) {
        await createNotification(
          ...
          `${leave.employee.name} has requested ${leaveType} leave from ${start.toLocaleDateString()} to ${end.toLocaleDateString()}.`,
          'LEAVE_APPROVAL'
        );
      }
    }
...
export const cancelLeave = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role !== Role.ADMIN && leave.employeeId !== req.user?.employeeId) {
...
export const approveLeave = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role === Role.MANAGER && leave.employee.managerId !== req.user.employeeId) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (leave.leaveType !== LeaveType.WFH) {
        const balanceField = {
          [LeaveType.CASUAL]: 'casualBalance',
          [LeaveType.SICK]: 'sickBalance',
          [LeaveType.EARNED]: 'earnedBalance',
        }[leave.leaveType];
        ...
        if (balanceField) {
          ...
          const currentBalance = (currentEmp as any)[balanceField];
    ...
    await createNotification(
      ...
      'Leave Approved',
      `Your request for leave from ${leave.startDate.toLocaleDateString()} to ${leave.endDate.toLocaleDateString()} has been approved.`,
      'LEAVE_APPROVAL'
    );
...
export const rejectLeave = async (req: Request, res: Response) => {
  try {
    ...
    await createNotification(
      ...
      `Your request for leave from ${leave.startDate.toLocaleDateString()} to ${leave.endDate.toLocaleDateString()} was rejected.`,
      'LEAVE_APPROVAL'
    );
